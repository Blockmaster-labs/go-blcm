...
// the process. This is usually the case when blcmis shutting down and some events
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but blcmis shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until blcmrequests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
