...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.SnapSync {
		...
		// txlookup limit.
		// The main concern here is: during the snap sync blcmwon't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next snap sync(e.g. user kill blcmmanually and
		// restart) then it will be hard for blcmto figure out the oldest block
