...
var (
	...
	IgnoreLegacyReceiptsFlag = &cli.BoolFlag{
		...
		Usage:    "blcmwill start up even if there are legacy receipts in freezer",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(RopstenFlag.Name) {
			// Maintain compatibility with older blcmconfigurations storing the
...
// for blcmand returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `blcmaccount list`)")
	log.Warn("-------------------------------------------------------------------")
