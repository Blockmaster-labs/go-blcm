...
The general flow for signing a transaction using e.g. blcmis as follows:
![image](sign_flow.png)
...
* blcmtodos
    ...
    - Relay: blcmshould be started in `blcm--signer localhost:8550`.
    - Currently, the blcmAPIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This type is 20 `bytes`, and is incapable of carrying checksum information. The signer uses `common.MixedcaseAddress`, which retains the original input.
    - The blcmAPI should switch to use the same type, and relay `to`-account verbatim to the external API.
* [x] Storage
