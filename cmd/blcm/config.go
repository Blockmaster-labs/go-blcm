...
// makeConfigNode loads blcmconfiguration and creates a blank node instance.
...
// makeFullNode loads blcmconfiguration and creates the Ethereum backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `blcmdb freezer-migrate`.")
		}
	}
