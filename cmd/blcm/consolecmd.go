...
var (
	...
	consoleCommand = &cli.Command{
		...
		Description: `
The blcmconsole is an interactive shell for the JavaScript runtime environment
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://geth.ethereum.org/docs/interface/javascript-console.
This command allows to open a console on a running blcmnode.`,
	}
...
// localConsole starts a new blcmnode, attaching a JavaScript console to it at the
// same time.
...
// remoteConsole will connect to a remote blcminstance, attaching a JavaScript
// console to it.
...
// ephemeralConsole starts a new blcmnode, attaches an ephemeral JavaScript
...
blcm--exec "%s" console`, b.String())
	return nil
}

// dialRPC returns a RPC client which connects to the given endpoint.
// The check for empty endpoint implements the defaulting logic
// for "blcmattach" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with blcm< 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
