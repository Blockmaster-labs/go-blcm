...
func doArchive(cmdline []string) {
	...
	var (
		env      = build.Env()
		baseblcm= archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		blcm    = "geth-" + baseblcm+ ext
		alltools = "geth-alltools-" + baseblcm+ ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(blcm, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{blcm, alltools} {
...
func doDocker(cmdline []string) {
	...
	//  - ethereum/client-go:latest                            - Pushes to the master branch, blcmonly
	//  - ethereum/client-go:stable                            - Version tag publish on GitHub, blcmonly
	//  - ethereum/client-go:alltools-latest                   - Pushes to the master branch, blcm& tools
	//  - ethereum/client-go:alltools-stable                   - Version tag publish on GitHub, blcm& tools
	//  - ethereum/client-go:release-<major>.<minor>           - Version tag publish on GitHub, blcmonly
	//  - ethereum/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, blcm& tools
	//  - ethereum/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, blcmonly
	//  - ethereum/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, blcm& tools
	var tags []string
	...
	if len(*manifest) != 0 {
		...
		// Assemble and push the blcmmanifest image
		for _, tag := range tags {
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	// first section contains the blcmbinary, second section holds the dev tools.
